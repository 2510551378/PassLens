...
const { buildDriverArgs, parseArgs, resolveDriverPath } = require('../scripts/torch-mlir-case-study-smoke.js');

test('parseArgs accepts torch-mlir case study smoke flags', () => {
  const previousDriver = process.env.PASS_LENS_TORCH_MLIR_DRIVER;
  const previousPipeline = process.env.PASS_LENS_TORCH_MLIR_PIPELINE;
  const previousCaseDir = process.env.PASS_LENS_TORCH_MLIR_CASE_DIR;
  const previousTimeout = process.env.PASS_LENS_TORCH_MLIR_TIMEOUT_MS;
  try {
    process.env.PASS_LENS_TORCH_MLIR_DRIVER = '/opt/torch-mlir/bin/driver';
    process.env.PASS_LENS_TORCH_MLIR_PIPELINE = 'builtin.module(func.func(canonicalize))';
    process.env.PASS_LENS_TORCH_MLIR_CASE_DIR = '/tmp/torch';
    process.env.PASS_LENS_TORCH_MLIR_TIMEOUT_MS = '42000';
    ...
    if (previousDriver === undefined) {
      ...
      process.env.PASS_LENS_TORCH_MLIR_DRIVER = previousDriver;
    }
    if (previousPipeline === undefined) {
      delete process.env.PASS_LENS_TORCH_MLIR_PIPELINE;
    } else {
      process.env.PASS_LENS_TORCH_MLIR_PIPELINE = previousPipeline;
    }
    if (previousCaseDir === undefined) {
      delete process.env.PASS_LENS_TORCH_MLIR_CASE_DIR;
    } else {
      process.env.PASS_LENS_TORCH_MLIR_CASE_DIR = previousCaseDir;
    }
    if (previousTimeout === undefined) {
      delete process.env.PASS_LENS_TORCH_MLIR_TIMEOUT_MS;
    } else {
      process.env.PASS_LENS_TORCH_MLIR_TIMEOUT_MS = previousTimeout;
    }
  }
});
...
test('resolveDriverPath resolves explicit executable paths', () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-driver-explicit-'));
  try {
    const driver = path.join(tmpRoot, 'torch-case-driver');
...
test('resolveDriverPath finds driver from PATH', () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'torch-driver-path-'));
  const originalPath = process.env.PATH;
  const originalPathExt = process.env.PATHEXT;
  try {
    const command = 'torch-pass-lens-driver';
...
test('mock torch-mlir downstream driver passes structured smoke checks', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'pass-lens-torch-mock-'));
  try {
    ...
    const caseName = 'mock-torch-downstream';
...
const trace = {
  ...
  provenance: { kind: 'live-pass-instrumentation', description: 'mocked torch-mlir downstream run' },
...
`;
    ...
    const result = spawnSync(process.execPath, [
      path.join(process.cwd(), 'scripts', 'torch-mlir-case-study-smoke.js'),
