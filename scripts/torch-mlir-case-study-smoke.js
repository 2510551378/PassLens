...
function parseArgs(argv) {
  const options = {
    caseName: undefined,
    checkArtifacts: true,
    driver: process.env.PASS_LENS_TORCH_MLIR_DRIVER,
    driverArgs: [],
    help: false,
    inputPath: process.env.PASS_LENS_TORCH_MLIR_INPUT,
...
function resolveDriverPath(rawDriver) {
  ...
  if (!command) {
    throw new Error('PASS_LENS_TORCH_MLIR_DRIVER or --driver is required.');
  }
...
function printSummary(summary) {
  console.log(`torch-mlir case study output: ${summary.outputRoot}`);
...
function printUsage() {
  console.log(`Usage: node scripts/torch-mlir-case-study-smoke.js --driver <path> [options]
